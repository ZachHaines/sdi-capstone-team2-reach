...
const ResourcePage = () => {
  ...
  let header = {height: '7vh', backgroundColor: bgcolor}
  ...
  return (
    <>
      <Paper sx={{backgroundColor: '#FFDDD2', width: '85vw'}}>
        <h1 style={{textAlign: 'center', ...background, borderRadius: '6px', color: '#EDF6F9'}}>Resources</h1>
          ...
          {facilities.map(facility => {
            return (
              <Grid key={facility.id} item xs={50} sm={25} md={20} sx={{padding: '1vh'}}>
                <Card sx={{height: '20vh', minWidth: '15vh', width: 'auto', backgroundColor: '#EDF6F9', border: '2px solid #83C5BE'}}>
                  <h3 style={{marginTop: '0', ...header}}><a href={facility.url} style={{color: '#EDF6F9'}}>{facility.name}</a></h3>
